  ...
  module Api
    class RotuloDownloadTest < ActiveSupport::TestCase
      ...
      test "raises TransientError when PPN-291 reports the label is not generated yet" do
        stub_request(:get, URL).to_return(
          status: 200,
          body: { "mensagem" => "PPN-291: O rótulo ainda não foi gerado. Por favor, consulte novamente." }.to_json,
          headers: { "Content-Type" => "application/json" }
        )

        error = assert_raises(Correios::Api::TransientError) { Correios::Api::RotuloDownload.fetch(RECIBO) }
        assert_match "PPN-291", error.message
      end

      test "raises LabelGenerationFailedError when PPN-295 reports the generation failed" do
        stub_request(:get, URL).to_return(
          status: 200,
