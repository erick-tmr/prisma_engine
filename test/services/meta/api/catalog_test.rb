...
module Meta
  module Api
    class CatalogTest < ActiveSupport::TestCase
      ...
      DATA = { title: "Zelda", price: 19_990, currency: "BRL" }.freeze
      ...
      test "upsert POSTs an UPDATE request and returns the parsed body" do
        stub_request(:post, URL).to_return(
          status: 200,
          body: { "handles" => [ "h1" ] }.to_json,
          headers: { "Content-Type" => "application/json" }
        )

        body = with_credentials { Meta::Api::Catalog.upsert("42", DATA) }
      ...
      test "treats an empty 200 body as an empty hash" do
        ...
        assert_equal({}, with_credentials { Meta::Api::Catalog.upsert("42", DATA) })
      end

      test "raises PermanentError on an unparseable 200 body" do
        stub_request(:post, URL).to_return(status: 200, body: "not json")
        assert_raises(Meta::Api::PermanentError) { with_credentials { Meta::Api::Catalog.upsert("42", DATA) } }
      end
      ...
      test "raises TransientError on a 4xx carrying the batch rate-limit code 80014" do
        ...
        assert_raises(Meta::Api::TransientError) { with_credentials { Meta::Api::Catalog.upsert("42", DATA) } }
      end
