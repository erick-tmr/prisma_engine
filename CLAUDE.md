...
- `application.html.erb` emits `javascript_importmap_tags "storefront_bootstrap"` — a **dedicated entry** that imports only Bootstrap's ESM build (the FAQ collapse is the lone Bootstrap JS component in use). The `application`/Turbo/Stimulus pins in `config/importmap.rb` stay **inert** (listed but never imported), so Turbo/Stimulus do not boot on the storefront. Don't switch the emitted entry to `application`. Bootstrap's classic bundle is no longer loaded; a new dropdown/modal/tooltip must be imported in `app/javascript/storefront_bootstrap.js`.
