...
- `bin/rails shipping:uncatalogued`: lists Correios tracking event codes we have observed but do not map in `Shipping::TrackingUpdate::EVENT_SIGNALS`, with occurrence and shipment counts. Reads the database by default (authoritative: every fetched event is persisted). `LOGS='/path/to/production.log*'` reads fetched log files instead, for when the database is out of reach. Exits non-zero when anything is uncatalogued, so it works as a periodic check. See **The tracking catalogue is an allowlist**.
...
- Catalog images are gitignored, not committed; production was bootstrapped once and the catalog is managed from the backoffice since. Runbook: `docs/seeding-images.md`.
- One-off scripts that tunnel to the production DB must guard the tunnel: `ssh -o ExitOnForwardFailure=yes`, a check that the local forward port is free, and a `SELECT current_database()` assertion before writing. `ssh -L` binds a port on *your* machine, and a collision with another project's Postgres silently points production credentials at that database. Do a read-only pass first and state what the rows will *become*, not just which rows are affected. **Delete the script once it has run**, so `deploy/` holds none of them at rest. Skeleton + rationale: `docs/deployment.md` § One-off data scripts.
...
- **An Order row exists only if InfinitePay minted its payment link first.** `Checkout::PlaceOrder` / `Checkout::PlaceMergeOrder` build the order, its items, the shipment and (for a merge) the `OrderMerge` plan **in memory**, mint the link through the `payment_link` callable the controller injects, and only then `save!`, which writes the whole graph in one Active Record transaction. A rejected link therefore leaves nothing behind. `Order#assign_number` is public for this reason (the payload needs `order_nsu` before the INSERT; `webhook_token` already exists on `Order.new`), and `Payments::Checkout` must stay attribute-only so it keeps working on an unsaved order. Do not move the write back ahead of the link: eleven orphaned orders in one sitting is what that cost a customer on 2026-08-11.
- **The payment webhook body is never trusted, and no order advances on it alone.** InfinitePay publishes `webhook_url` (token and all) in the JavaScript of their own hosted payment page, so anyone holding a checkout link can forge a delivery to `POST /pagamentos/webhook/:token`. The controller still acks every delivery with `head :ok`, but the body only names a transaction: `Payments::ProcessWebhookJob` runs `Payments::Verification` first, which asks InfinitePay over `POST /payment_check` (`InfinitePay::Api::PaymentCheck`) and hands `Payments::PaymentUpdate` a payload whose `paid_amount` and `capture_method` come from **that response**, keyed on **our** `order.number` rather than the delivery's `order_nsu`. Do not wire `PaymentUpdate` straight to a webhook payload again, and do not let the delivery's own figures back into the confirmation path. `Checkout::ReturnsController#record_transaction` runs the same check before trusting the redirect's query params. It fails closed on purpose: a delivery with no `invoice_slug` never confirms. An unpaid verdict is re-checked on a bounded schedule (`Payments::VERIFY_*`, 30s/60s/120s) and then dropped with a log line, never a failed execution.
...
- `Meta::Api::Client` and `InfinitePay::Api::Client` use the same middleware for the same reason. InfinitePay carries **no filter at all**: it sends no `Authorization` header (the account is identified by `handle` in the body), and the payload plus the response body are logged byte-exact so a rejected link is diagnosable. That includes the customer's name, e-mail, phone and address, and the `webhook_url` with the `Order#webhook_token` that authenticates `POST /pagamentos/webhook/:token`. Deliberate: production logs are private, and a filtered payload is unreadable exactly when it matters. `Payments::Checkout#start` additionally logs `[Payments::Checkout] order=... link rejected: <status + vendor body>` before re-raising, so every rejection is greppable by order.

### The label pipeline is a saga

`Shipping::EmitLabel.resume(shipment)` reads the persisted `ShippingLabel#state` and enqueues the job for that step. **The saga is keyed on the shipment, not the order**, because an order has two of them: an `outbound` one and, once a return is authorized, an `inbound` one (`Shipment#direction`, unique per `[order_id, direction]`). The four step jobs take a **required keyword** (`perform(shipment_id:)`) rather than a positional id, so a job left in Solid Queue across the deploy that introduced this fails loudly with `ArgumentError` instead of silently re-reading an order id as a shipment id; keep it that way if you ever re-key them again. `EmitLabelJob` / `EmitLabelsJob` stay positional and order-keyed on purpose: they mean "issue the outbound label for these orders", which did not change. Everything asymmetric between the two directions lives in `Shipping::Leg` and nowhere else. It is safe to call at any time, from anywhere, any number of times: progress lives in the database, never in an in-memory handoff, so a replay after a crash, a deploy, a Correios timeout, or a repeated operator click converges on the same result. **Do not add deduplication to `resume`, and do not treat repeated enqueues as a correctness bug.** Re-deriving the next step from stored state is the mechanism that makes the chain resumable.
...
### The tracking catalogue is an allowlist

`Shipping::TrackingUpdate::EVENT_SIGNALS` maps a Correios `(code, type)` pair to one of our signals, and `MOVEMENT_SIGNALS` names the subset that means the package physically moved. **Only a catalogued movement signal advances an order.** An uncatalogued code logs `[correios-rastro] unmapped event` and changes nothing.

It used to be a denylist (`signal(event) != :label_issued`), which meant every code we had never seen counted as movement. On 2026-08-14 Correios sent `FC/83` "Etiqueta expirada" for two pré-postagens that were printed but never handed over; the sync read the label's death certificate as a posting, walked PG-11694 and PG-31710 to `shipped`, stamped `posted_at` with the moment the label died, and e-mailed both customers that their order had been posted. Do not restore a catch-all: an unknown code stalling an order where an operator can see it beats a false "postado" e-mail to a customer.

The trade is that a genuinely new movement code stalls orders until it is mapped, so the catalogue has to be swept. That is what `bin/rails shipping:uncatalogued` is for.

`FC/83` additionally marks the shipment's pré-postagem `expirado` (`Shipment#expire_prepost`), which drops it out of `Shipment.awaiting_tracking` so a dead label stops being polled forever, and surfaces as an `expired` state on the backoffice label card.

**Recovering an expired label is `Shipping::ReissueLabel`, not `EmitLabel.restart`.** The pré-postagem is gone, so the shipment needs a brand new one, and `ShipmentFactory` writes only `where(pre_post_id: nil)` (its guard against a duplicate `CreatePrePostagemJob` clobbering a live pré-postagem). Re-emitting without clearing that column therefore *buys* a pré-postagem at Correios, discards the response as a duplicate, and leaves the shipment on the dead one, with `validate_pre_postagem!` still passing because the old values are populated. `ReissueLabel` clears the pré-postagem columns, the tracking read-model and the stored events, resets the label to `pending` (dropping the void rótulo PDF and recibo so it cannot be printed), then calls `EmitLabel.resume`. It is guarded on `label_expired?`, so a second click cannot buy a second rótulo. `reset_for_relabel!` is a different thing: it targets `prepost_confirmed`, meaning the pré-postagem is alive and only the rótulo needs re-requesting.

The shipment row is reused rather than rebuilt: `shipments` is unique on `[order_id, direction]`, and the outbound row carries the address, service and price snapshot the customer paid for at checkout. The order stays on `label_issued` throughout (`advance_to_label_issued!` no-ops when already there), so the customer is not e-mailed twice. It is **operator-triggered only** (`POST /admin/etiquetas/:number/reemitir`, the button on the expired card): re-emission spends money and the package may no longer be ready to ship, so no job does it automatically.

`shipments.posting_deadline` holds Correios' `prazoPostagem`, the exact date the pré-postagem expires, captured when the label is bought and **currently read by nothing**. Both 2026-08-14 expiries were knowable 14 days ahead from that column. Watching it is the fix that prevents this rather than recovering from it.
